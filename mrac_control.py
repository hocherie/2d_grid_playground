...
class AdaptNet():
    def __init__(self):
        ...
        self.V = np.zeros((self.n_in + 1, self.n_hid)) # plus one to include theta row
        self.W = np.zeros((self.n_hid + 1, self.n_out))  # plus one to include theta row
    ...
    def forward(self, X):
        ...
        # input to hidden
        z1 = self.V.T @ X_bar + self.bv * self.theta_v  # 10x 1 # TODO: should it be transposed?
    ...
    def compute_vgrad(self, X_in, Rp, Rd, track_error):
        ...
        # Final compute (eq. 52)
        first_inner = X_bar @ (r.T @ self.W.T @ self.sigmoid_p_bw(self.V.T @ X_bar))
        second_inner = self.k_track * np.linalg.norm(track_error) * self.V #TODO: @ or *
